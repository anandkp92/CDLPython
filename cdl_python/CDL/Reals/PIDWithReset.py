# ABOUTME: PIDWithReset block - PID controller with anti-windup and reset capability.
# ABOUTME: Extends standard PID with ability to reset integrator via trigger input.
...
class PIDWithReset(CDLBlock):
    """
    P, PI, PD, and PID controller with integrator reset.
    ...
    Adds reset capability: when trigger input is true, integrator is reset to y_reset_in.
    ...
    Inputs:
        ...
        trigger: Reset trigger (when true, reset integrator)
        y_reset_in: Value to reset integrator to
    ...
    def __init__(
        ...
        """
        Initialize PID controller with reset.
    ...
    def compute(self, u_s: float, u_m: float, trigger: bool = False, y_reset_in: float = 0.0) -> Dict[str, Any]:
        """
        Compute PID controller output with reset capability.

        Args:
            u_s: Setpoint
            u_m: Measurement
            trigger: Reset trigger (when true, reset integrator)
            y_reset_in: Value to reset output to
        ...
        if trigger and not self._state['last_trigger']:
            # Reset integrator
            if self.with_I:
                # Reset integral to achieve y_reset_in
                self._state['integral'] = y_reset_in * self.Ti / self.k
